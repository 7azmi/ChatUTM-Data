...
# Initialize OpenAI GPT-4o Mini API client
OPENAI_API_KEY = os.getenv('OPENAI_API_KEY')  # Use your OpenAI API key
client = OpenAI(api_key=OPENAI_API_KEY)  # No need for base_url
...
Ignore casual greetings, posters, announcements, media omissions, missing or incomplete answers, or non-informative exchanges.
"""

def call_openai_api(chat_text):
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",  # Use GPT-4o Mini instead of DeepSeek
            messages=[
...
def process_single_file(file_path):
    ...
    for i, chunk in enumerate(chunks):
        ...
        json_response = call_openai_api(chunk)
